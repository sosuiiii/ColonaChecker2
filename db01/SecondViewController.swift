...
class SecondViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        customTableView.register(UINib(nibName: "CustomTableViewCellSecond", bundle: nil), forCellReuseIdentifier: "customCell")
        let realm = try! Realm()
        let pre = realm.objects(Preference.self)
        highestCase = pre[12].pcr
        ...
        segment.setTitle("PCR(人)降順", forSegmentAt: 1)
        segment.setTitle("PCR(人)昇順", forSegmentAt: 2)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "customCell", for: indexPath) as! CustomTableViewCellSecond
        ...
        let num = UILabel()
        num.text = String(pre![indexPath.row].pcr)
        ...
        progressView.setProgress(Float(pre![indexPath.row].pcr) / Float(highestCase!), animated: false)
